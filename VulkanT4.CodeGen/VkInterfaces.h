...
namespace ManagedVulkan 
{
	...
	public:
		...
		VkResult MapMemory(DeviceMemory memory, UInt64 offset, UInt64 size, VkMemoryMapFlags flags, IntPtr% ppData);
		...
		void GetDeviceMemoryCommitment(DeviceMemory memory, UInt64% pCommittedMemoryInBytes);
		...
		VkResult GetPipelineCacheData(PipelineCache pipelineCache, IntPtr% pDataSize, IntPtr pData);
		...
		VkResult AcquireNextImageKHR(SwapchainKHR swapchain, UInt64 timeout, Semaphore semaphore, Fence fence, UInt32% pImageIndex);
	};
	...
	public:
		...
		VkResult CreateDisplayModeKHR(DisplayKHR display, VkDisplayModeCreateInfoKHR* pCreateInfo, AllocationCallbacks^ pAllocator, out DisplayModeKHR^ pMode);
	...
	public:
		...
		void CmdUpdateBuffer(Buffer dstBuffer, UInt64 dstOffset, UInt64 dataSize, UInt32% pData);
